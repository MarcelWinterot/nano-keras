...
class Layer:
    ...
    def __init__(self, units: int, activation: Activation | str, weight_initialization: Initializer | str = "random_normal", bias_initialization: Initializer | str = "random_normal", regulizer: Regularizer = None, trainable: bool = True, input_shape: tuple = None, name: str = "Layer") -> None:
    ...
    def set_batch_size(self, batch_size: int, layers: list, index: int) -> None:
        ...
        self.inputs = np.ndarray((self.batch_size, *input_shape)) if isinstance(input_shape, tuple) else np.ndarray(
            (self.batch_size, input_shape))

        self.outputs = np.ndarray((self.batch_size, *output_shape)) if isinstance(output_shape, tuple) else np.ndarray(
    ...
    def generate_weights(self, layers: list, current_layer_index: int, weight_data_type: np.float_, bias_data_type: np.float_) -> None:
        ...
        if self.input_shape:
            # I've observed this gives us better results
            np.random.randn(*self.input_shape) if isinstance(
                self.input_shape, tuple) else np.random.randn(self.input_shape)
